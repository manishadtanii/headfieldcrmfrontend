...
import {
  ...
  RefreshCw, Globe, Mail, Building2, Clock, AlertTriangle,
} from 'lucide-react';
...
const ROLE_CONFIG = {
  businessAdmin: { label: 'Biz Admin', color: '#818cf8', bg: '#818cf818' },
  employee:      { label: 'Employee',  color: '#34d399', bg: '#34d39918' },
};
...
const ConfirmModal = ({ config, loading, onConfirm, onCancel }) => (
  ...
  >
    <div
      style={{ background: 'var(--bg-card)', border: `1px solid var(--border)`, borderRadius: 20, width: '100%', maxWidth: 420, overflow: 'hidden', boxShadow: `0 24px 60px rgba(0,0,0,0.4), 0 0 0 1px ${config.accentColor}20` }}
      onClick={e => e.stopPropagation()}
    >
      <div style={{ height: 4, background: config.accentColor }} />
      <div style={{ padding: '28px 28px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div style={{ width: 56, height: 56, borderRadius: 14, background: config.iconBg, border: `1px solid ${config.iconBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {config.icon}
          </div>
          <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 20, background: config.badgeBg, color: config.badgeColor, border: `1px solid ${config.badgeColor}30` }}>
            {config.badge}
          </span>
        </div>
        ...
        <div style={{ background: 'var(--bg-elevated)', borderRadius: 10, padding: '14px 16px', border: '1px solid var(--border)', marginBottom: 20 }}>
          {config.lines.map((line, i) => (
            <p key={i} style={{ fontSize: 13, color: i === 0 ? 'var(--text)' : 'var(--text-muted)', lineHeight: 1.6, marginBottom: i < config.lines.length - 1 ? 8 : 0 }}
              dangerouslySetInnerHTML={{ __html: line }} />
          ))}
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onCancel} style={{ flex: 1, padding: '11px 0', borderRadius: 10, fontSize: 13, fontWeight: 600, background: 'var(--bg-elevated)', border: '1px solid var(--border)', color: 'var(--text)', cursor: 'pointer' }}>
            Cancel
          </button>
...
// ── Credentials Box (after create / reset) ────────────────────────
const CredentialsBox = ({ creds, onClose }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      onClick={onClose}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 20, width: '100%', maxWidth: 460, overflow: 'hidden', boxShadow: '0 24px 60px rgba(0,0,0,0.4)' }}
        onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: '24px 26px 20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 40, height: 40, borderRadius: 10, background: '#34d39918', border: '1px solid #34d39930', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Check size={18} color="#34d399" />
              </div>
              <div>
                <div style={{ fontWeight: 800, fontSize: 16 }}>Credentials Ready</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>Save these — password won't show again</div>
              </div>
            </div>
          ...
          <div style={{ background: 'var(--bg-elevated)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden', marginBottom: 16 }}>
            {[
              ...
              { key: 'Password', icon: <KeyRound size={13} />, val: creds.tempPassword, mono: true, sensitive: true },
              ...(creds.loginUrl ? [{ key: 'Login URL', icon: <Globe size={13} />, val: creds.loginUrl, mono: true }] : []),
            ].map(({ key, icon, val, mono, sensitive }, i, arr) => (
              <CredRow key={key} label={key} icon={icon} value={val} mono={mono} sensitive={sensitive} divider={i < arr.length - 1} />
            ))}
          </div>

          {/* Actions */}
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={copyAll} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '10px 0', borderRadius: 10, fontSize: 13, fontWeight: 600, background: 'var(--bg-elevated)', border: '1px solid var(--border)', color: 'var(--text)', cursor: 'pointer' }}>
              <Copy size={13} /> Copy All
            </button>
            <button onClick={onClose} style={{ flex: 1, padding: '10px 0', borderRadius: 10, fontSize: 13, fontWeight: 700, background: 'linear-gradient(135deg,#34d399,#6ee7b7)', border: 'none', color: 'white', cursor: 'pointer' }}>
              Done
            </button>
          </div>
        </div>
  ...
  return (
    <div style={{ padding: '11px 14px', borderBottom: divider ? '1px solid var(--border)' : 'none', display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
        {sensitive && (
          <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', padding: 3 }} onClick={() => setShow(s => !s)}>
            {show ? <EyeOff size={12} /> : <Eye size={12} />}
          </button>
        )}
        <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', padding: 3 }}
...
// ── Password mini cell (in table) ─────────────────────────────────
const PasswordCell = ({ password }) => {
  const [show, setShow] = useState(false);
  if (!password) return <span style={{ color: 'var(--text-muted)', fontSize: 12 }}>—</span>;
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
      <span style={{ fontFamily: 'monospace', fontSize: 12, letterSpacing: 0.5 }}>{show ? password : '••••••••'}</span>
      <button className="btn btn-ghost btn-icon" style={{ padding: 3 }} onClick={() => setShow(s => !s)}>
        {show ? <EyeOff size={11} /> : <Eye size={11} />}
      </button>
      <button className="btn btn-ghost btn-icon" style={{ padding: 3 }} onClick={() => { navigator.clipboard.writeText(password); toast.success('Password copied!'); }}>
        <Copy size={11} />
      </button>
    </div>
  );
...
// ── User Row ──────────────────────────────────────────────────────
const UserRow = ({ user, onToggle, onLogout, onResetPW }) => {
  const rc = ROLE_CONFIG[user.role] || { label: user.role, color: '#6b7280', bg: '#6b728018' };
  const loginUrl = user.business?.slug
    ? `${window.location.origin}/${user.business.slug}/login`
    : null;
  ...
  return (
    ...
    >
      ...
      <td style={{ padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{
            ...
            background: user.role === 'businessAdmin' ? 'linear-gradient(135deg,#818cf8,#a78bfa)' : 'linear-gradient(135deg,#34d399,#6ee7b7)',
      ...
      {/* Role */}
      <td style={{ padding: '14px 16px' }}>
        <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: rc.bg, color: rc.color, border: `1px solid ${rc.color}30` }}>
          {rc.label}
        </span>
      </td>
      ...
      <td style={{ padding: '14px 16px' }}>
        {user.business ? (
          <div>
            <div style={{ fontWeight: 600, fontSize: 13 }}>{user.business.name}</div>
            {loginUrl && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 4 }}>
                <Globe size={10} color="var(--text-muted)" />
                <span style={{ fontSize: 11, color: '#818cf8', fontFamily: 'monospace' }}>/{user.business.slug}/login</span>
                <button
      ...
      <td style={{ padding: '14px 16px' }}>
        <span style={{
          ...
          background: user.isActive ? '#34d39918' : '#ef444418',
          color: user.isActive ? '#34d399' : '#ef4444',
          border: `1px solid ${user.isActive ? '#34d39930' : '#ef444430'}`,
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          {user.isOnline && (
            <button className="btn btn-ghost btn-icon btn-sm" title="Force Logout"
              onClick={() => onLogout(user)} style={{ color: '#fbbf24' }}>
              <LogOut size={14} />
            </button>
          )}
          ...
          <button className="btn btn-ghost btn-icon btn-sm" title="Reset Password"
            onClick={() => onResetPW(user)} style={{ color: '#818cf8' }}>
            <KeyRound size={14} />
          </button>
        </div>
      </td>
...
export default function SAUsers() {
  ...
  const buildConfig = (type, user) => {
    if (type === 'toggle') {
      ...
      return {
        ...
        iconBg:       isDeac ? '#f59e0b18' : '#34d39918',
        iconBorder:   isDeac ? '#f59e0b30' : '#34d39930',
        badge:        isDeac ? 'Reversible Action' : 'Restoring Access',
        badgeBg:      isDeac ? '#f59e0b15' : '#34d39915',
        badgeColor:   isDeac ? '#f59e0b' : '#34d399',
        title:        isDeac ? 'Deactivate User' : 'Activate User',
        lines: isDeac
          ? [`Deactivating <strong>${user.name}</strong>.`, 'They will lose login access immediately. All their data stays safe.']
    ...
    if (type === 'logout') return {
      ...
      title: 'Force Logout',
      lines: [`Force logging out <strong>${user.name}</strong>.`, 'Their session will be terminated immediately. They can log back in unless deactivated.'],
    ...
    if (type === 'reset') return {
      ...
      lines: [`Reset password for <strong>${user.name}</strong>?`, 'A new temporary password will be generated. The user must change it on first login. Share credentials securely.'],
  ...
  const onlineN  = users.filter(u => u.isOnline).length;
    ...
    <div className="page-content">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
        {[
          ...
          { icon: Wifi,   label: 'Online Right Now', value: onlineN, color: '#fbbf24', grad: 'linear-gradient(90deg,#fbbf24,#f59e0b)' },
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '12px 16px' }}>
        ...
        />
        {search && (
          <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex' }} onClick={() => { setSearch(''); setPage(1); }}>
            <X size={14} />
          </button>
        )}
        <div style={{ width: 1, height: 20, background: 'var(--border)' }} />
        <select
          style={{ background: 'none', border: 'none', outline: 'none', fontSize: 13, color: 'var(--text)', cursor: 'pointer' }}
          value={filterRole}
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border)' }}>
              {['User', 'Role', 'Business & Login URL', 'Password', 'Status', 'Online', 'Last Login', 'Actions'].map(h => (
                <th key={h} style={{ padding: '11px 16px', textAlign: 'left', fontWeight: 600 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={8}>
                <div style={{ padding: '60px 0', textAlign: 'center' }}>
                  ...
                  <div style={{ fontWeight: 700, marginBottom: 6 }}>No users found</div>
                  <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>{search ? `No results for "${search}"` : 'Create your first user above.'}</div>
                </div>
              </td></tr>
            ) : users.map(user => (
              <UserRow
        ...
        {/* Pagination */}
        {total > LIMIT && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 18px', borderTop: '1px solid var(--border)', background: 'var(--bg-elevated)' }}>
      ...
      {confirm && (
        <ConfirmModal
          config={buildConfig(confirm.type, confirm.user)}
